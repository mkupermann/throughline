# Shared-workspace acceptance record
...
| Recovery | Migration from current installation, separate-database restore, retained conversations and secrets | Restored a private snapshot of the regular installation and applied migrations 013–015: original conversation/message counts, original message contents and provider-key values preserved; the rehearsal did not modify the live database |
...
| Regular installation | Upgrade without changing source mounts, messages, provider secrets or purpose bindings | Migrations 013–015 and the verified application image deployed successfully; original row counts, message contents, provider keys and purpose bindings preserved; PostgreSQL not restarted. Current frontend, project history and synthetic Codex/Vibe tests passed. Local mode remains enabled; team mode is opt-in. |
