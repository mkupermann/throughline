...
function ConversationReader({ project, id, session, scope }: {project: string; id: number; session?: StorySession; scope: URLSearchParams}) {
  ...
  return <section className="conversation-reader" aria-label={t("Selected conversation")}>
    ...
    {!!detail.data?.pages[0].artifacts?.length && <section className="conversation-artifacts"><h3>{t("Referenced files")}</h3><p>{t("Availability is checked on this server; file contents and success are not verified.")}</p>{detail.data.pages[0].artifacts.map((a, index) => <article key={`${a.message_id}:${a.path}`}><strong>{a.label}</strong><p><code>{a.path}</code></p><Timestamp value={a.created_at} /> · <Link to={`/c/${id}#m${a.message_id}`}>{t("Source")}</Link><p>{a.availability === "available" ? t("Available on this server") : t("Not accessible on this server")}</p>{a.availability === "available" && <a className="button" href={`/api/story/${encodeURIComponent(project)}/session/${id}/artifact/${a.message_id}/${index}?${scope}`} download>{t("Download file")}</a>}</article>)}</section>}
