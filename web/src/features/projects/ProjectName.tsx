...
export function ProjectName({project, name, folders, origin, sources=[]}: {project: string; name?: string | null; folders: number; origin?: string; sources?: number[]}) {
  ...
  return <div className="project-naming">
    ...
    {!editing ? <button className="button" onClick={() => {setDraft(name ?? "");setEditing(true);}}>{t(name ? "Edit project name" : "Name this project")}</button> : <form onSubmit={async e => {
      e.preventDefault();setSaving(true);setError("");
      try {await projectsApi.rename(project,draft.trim());await Promise.all([client.invalidateQueries({queryKey:["project-library"]}),client.invalidateQueries({queryKey:["story"]})]);setEditing(false);}
      catch(e) {setError(e instanceof Error ? e.message : t("Could not save project name"));}
      finally {setSaving(false);}
    }}><label>{t("Project name")} <input autoFocus maxLength={120} required value={draft} onChange={e=>setDraft(e.target.value)} /></label>
    <p>{t("This label applies to this folder group. Renaming does not merge or reassign conversations.")}</p>
    <button className="button" disabled={saving || !draft.trim()}>{t(saving ? "Saving…" : "Save name")}</button> <button type="button" className="button" disabled={saving} onClick={()=>setEditing(false)}>{t("Cancel")}</button></form>}
